...
#include "toolbox/utils/Logging.h"
...
std::vector<std::string> GetPathComponents(const std::string &path) {
    ...
    std::vector<std::string> components;
    if (isRelative) {
        auto cwd = utils::GetWorkingDirectory();
...
bool DeleteDirectory(const std::string &directory) {
    ...
    if (fs::remove_all(directory, error) == static_cast<std::uintmax_t>(-1)) {
        utils::LogWarning("Failed to remove directory {}: {}.", directory,
                          error.message());
        return false;
    }
    return true;
...
bool Copy(const std::string &src_path, const std::string &dst_path) {
    try {
        ...
        utils::LogWarning("Failed to copy {} to {}. Exception: {}.", src_path,
                          dst_path, e.what());
        return false;
    }
    return true;
...
std::string AddIfExist(const std::string &path,
    ...
    for (const auto &folder_name : folder_names) {
        ...
        if (utils::DirectoryExists(folder_path)) {
            return folder_path;
        }
    }
    return path;
...
void CFile::Close() {
    if (file_) {
        if (fclose(file_)) {
            error_code_ = errno;
            utils::LogError("fclose failed: {}", GetError());
        }
        file_ = nullptr;
    }
}
...
int64_t CFile::CurPos() {
    if (!file_) {
        utils::LogError("CFile::CurPos() called on a closed file");
    }
    int64_t pos = ftell(file_);
    if (pos < 0) {
        error_code_ = errno;
        utils::LogError("ftell failed: {}", GetError());
    }
    return pos;
}
...
int64_t CFile::GetFileSize() {
    if (!file_) {
        utils::LogError("CFile::GetFileSize() called on a closed file");
    }
    fpos_t prevpos;
    if (fgetpos(file_, &prevpos)) {
        error_code_ = errno;
        utils::LogError("fgetpos failed: {}", GetError());
    }
    if (fseek(file_, 0, SEEK_END)) {
        error_code_ = errno;
        utils::LogError("fseek failed: {}", GetError());
    }
    int64_t size = CurPos();
    if (fsetpos(file_, &prevpos)) {
        error_code_ = errno;
        utils::LogError("fsetpos failed: {}", GetError());
    }
    return size;
}
...
int64_t CFile::GetNumLines() {
    if (!file_) {
        utils::LogError("CFile::GetNumLines() called on a closed file");
    }
    fpos_t prevpos;
    if (fgetpos(file_, &prevpos)) {
        error_code_ = errno;
...
const char *CFile::ReadLine() {
    if (!file_) {
        utils::LogError("CFile::ReadLine() called on a closed file");
    }
    ...
    if (!fgets(line_buffer_.data(), int(line_buffer_.size()), file_)) {
        if (ferror(file_)) {
            utils::LogError("CFile::ReadLine() ferror encountered");
        }
        if (!feof(file_)) {
            utils::LogError(
    ...
    if (strlen(line_buffer_.data()) == line_buffer_.size() - 1) {
        ...
        utils::LogError("CFile::ReadLine() encountered a line longer than {}",
                        line_buffer_.size() - 2);
    }
...
size_t CFile::ReadData(void *data, size_t elem_size, size_t num_elems) {
    if (!file_) {
        utils::LogError("CFile::ReadData() called on a closed file");
    }
    size_t elems = fread(data, elem_size, num_elems, file_);
    if (ferror(file_)) {
        utils::LogError("CFile::ReadData() ferror encountered");
    }
    if (elems < num_elems) {
        if (!feof(file_)) {
            utils::LogError(
